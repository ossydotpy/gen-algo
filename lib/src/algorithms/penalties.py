...
class Penalties:
    ...
    def __init__(self):
        self.penalty_functions = [
            (self.preference_penalty, 1),
            (self.same_day_subject_penalty, 2),
            # (self.subject_exhaustion_penalty, 2),
            # (self.balance_penalty, 1.5),
    ...
    def preference_penalty(
        ...
        for day, schedule in individual.timetable.items():
            for time, subject in schedule.items():
                if subject in preferences and preferences[subject].get(day) != time:
                    penalty += 1
        return penalty
    ...
    def same_day_subject_penalty(
        ...
        """Penalty for scheduling same subject multiple times a day"""
        penalty = 0
        for day, schedule in individual.timetable.items():
            subject_counts = {}
    ...
    #     )
    #     return (
    #         max(0, allocated_slots - len(subjects)) * 5
    #     )  # Penalize if more slots are filled than subjects
